...
class families extends model
{
	...
	private $table = '#_families';
    private $view  = 'families';
    private $key    = 'id';
	private $order  = 'ordering';
	private $dir  = 'ASC';
	private $sql    = 'SELECT i.*, s.color FROM `#_families` AS i INNER JOIN `#_salons` AS s ON i.idSalo = s.id';
	private $rows   = 'SELECT COUNT(i.id) FROM `#_families` AS i';
    ...
    */
    function saveItem()
    {
        ...
        if($result) {
        	$msg  = "La familia ha estat guardada";
        	$type = 'success';
        } else {
        	$msg  = "La familia no s'ha pogut guardar";
        	$type = 'danger';
        }
