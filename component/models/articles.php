...
class articles extends model
{
	...
	private $dir  = 'ASC';
	private $sql    = 'SELECT i.*FROM `#_articles` AS i ';
	private $rows   = 'SELECT COUNT(i.id) FROM `#_articles` AS i';
    ...
    */
    ...
    /**
     * Metode per carregar families segons el saló escollit
    */
    function getFamiliesBySalo() 
    {
        ...
        $db->query('SELECT id, descripcio FROM #_families WHERE idSalo = '.$id);
        $row = $db->fetchObjectList();
    ...
    */
    ...
    function getIVAProductes()
    {
        ...
        $db->query('SELECT * FROM #_tipus_iva WHERE id = '.$id);
    ...
    */
    function saveItem() 
    {
        ...
        if($result) {
        	$msg  = "L'article ha estat guardat";
        	$type = 'success';
        } else {
        	$msg  = "L'article no s'ha pogut guardar";
        	$type = 'danger';
        }
